      ...
      <f7-block strong v-if="addon.id">
        <f7-row>
          <f7-col v-if="addon.link">
            <f7-button outline color="blue" :href="addon.link" external target="_blank">Open documentation</f7-button>
          </f7-col>
          <f7-col>
            <f7-button outline fill color="blue" v-if="state === 'UNINSTALLED'" @click="install()">Install {{addon.version}}</f7-button>
            <f7-button outline fill color="red" v-if="state === 'INSTALLED'" @click="uninstall()">Uninstall {{addon.version}}</f7-button>
          </f7-col>
        </f7-row>
      </f7-block>
      <f7-block v-if="bindingInfo.description">
        <p><strong>Author: {{bindingInfo.author}}</strong></p>
...
export default {
        ...
        json.then((j) => {
          ...
          if (this.addon.type === 'binding' && this.addon.installed) {
            fetch('/rest/bindings').then((resp2) => {
              const json2 = resp2.json()
              json2.then((j2) => {
                this.bindingInfo = j2.find((b) => b.id === this.addonId.replace('binding-', '')) || {}
  ...
  methods: {
    install () {
      this.$f7.dialog.alert('Installing/uninstalling addons not implemented yet!', 'Sorry')
    },
    uninstall () {
      this.install()
    }
