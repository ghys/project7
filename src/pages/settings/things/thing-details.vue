<template>
  <f7-page>
    <f7-navbar :title="thing.label" back-link="Back">
		</f7-navbar>
    <f7-toolbar tabbar>
    ...
    <f7-tabs v-if="thing.UID">
      <f7-tab id="info" tab-active>
        ...
        <f7-block class="block-narrow" strong>
          <f7-col>
            Status: <f7-chip :text="thing.statusInfo.status" :color="thing.statusInfo.status === 'ONLINE' ? 'green' : 'red'">{{thing.statusInfo.status}}</f7-chip>
            <div v-if="thing.statusInfo.statusDetail !== 'NONE' || thing.statusInfo.description">
              <strong v-if="thing.statusInfo.statusDetail !== 'NONE'">{{thing.statusInfo.statusDetail}}</strong>
              <br />
        ...
        <f7-block class="block-narrow">
          <f7-col>
            ...
            <div v-html="thingType.description">
            </div>
          </f7-col>
        </f7-block>
            ...
            <f7-list accordion-list>
              <f7-list-item accordion-item title="Properties">
                <f7-accordion-content>
                  <f7-list>
                    <f7-list-item v-for="(value, key) in thing.properties" :key="key"
              ...
              <f7-list-item accordion-item title="Textual definition equivalent">
                <f7-accordion-content>
                  <f7-block>
                    ...
                    <p v-if="this.thing.bridgeUID">Attention: This Thing is provided by a Bridge (<strong>{{this.thing.bridgeUID}}</strong>). You can also include it within the Bridge block and remove the reference between parentheses.</p>
                    <code class="textual-definition"><pre>{{textualDefinition}}</pre></code>
                  </f7-block>
                </f7-accordion-content>
              </f7-list-item>
            </f7-list>
      ...
      <f7-tab id="config" v-if="thing.configuration && thingType.configParameters">
        <f7-block class="block-narrow">
          <f7-col v-if="!thingType.parameterGroups.length">
            <config-parameter v-for="parameter in thingType.configParameters" :key="parameter.name"
              :config-description="parameter" :value="thing.configuration[parameter.name]" />
          </f7-col>
          <f7-col v-if="thingType.parameterGroups.length">
            <f7-list accordion-list>
              <f7-list-item accordion-item
                v-for="group in thingType.parameterGroups"
                :key="group.name" :title="group.label">
                <f7-accordion-content>
                  <f7-block>
                    <config-parameter
          ...
          <f7-col v-if="thingType.channelGroups.length">
            <f7-list accordion-list>
              <f7-list-item accordion-item
                ...
                :key="group.id" :title="group.label" :footer="group.description">
                <f7-accordion-content>
...
export default {
  ...
  props: [
	...
	created () {
    copyToast = this.$f7.toast.create({ text: 'Textual definition copied to clipboard', closeTimeout: 2000 })
  ...
  computed: {
    textualDefinition () {
      ...
      if (this.thing.bridgeUID) {
        definition += '# Attention: This Thing is provided by a Bridge (' + this.thing.bridgeUID + ').' +
      ...
      for (let parameter in this.thing.configuration) {
        if (!Array.isArray(this.thing.configuration[parameter])) {
          parameters.push(parameter + '=' + JSON.stringify(this.thing.configuration[parameter]))
        }
      }
      ...
      for (let channel of this.thing.channels) {
        ...
        itemDefinition += ' ' + this.thing.label.replace(/[^0-9a-z]/gi, '') + '_' + channel.id.replace(/[^0-9a-z]/gi, '')
