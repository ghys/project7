<template>
  <f7-page>
    <f7-navbar title="Items" back-link="Back">
      <f7-nav-right>
        <f7-link class="searchbar-enable" data-searchbar=".searchbar-items" icon-ios="f7:search_strong" icon-md="material:search"></f7-link>
      </f7-nav-right>
				<f7-searchbar
          v-if="initSearchbar"
  ...
  methods: {
    searchAll(query, items) {
      ...
      for (let i = 0; i < items.length; i += 1) {
        ...
        if (haystack.toLowerCase().indexOf(query.toLowerCase()) >= 0 || query.trim() === '') found.push(i);
      }
