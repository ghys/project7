<template>
  <f7-page>
    ...
    <f7-block form v-if="configDescriptions && config" class="service-config block-narrow">
      <f7-col>
        <config-parameter v-for="parameter in configDescriptions.parameters" :key="parameter.name"
          :config-description="parameter" :value="config[parameter.name]" />
      </f7-col>
      <!-- <f7-list-input
    ...
    fetch('/rest/services/' + this.serviceId).then((resp) => {
      ...
      json.then((j) => {
        ...
        if (this.service.configDescriptionURI) {
          fetch('/rest/config-descriptions/' + this.service.configDescriptionURI).then((resp2) => {
            const json2 = resp2.json()
            json2.then((j2) => {
              this.configDescriptions = j2

              fetch('/rest/services/' + this.serviceId + '/config').then((resp3) => {
